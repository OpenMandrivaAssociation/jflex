...
# fix for building with jflex 1.4
sed -i 's/%inputstreamctor false//' src/main/jflex/LexScan.flex
...
install -d -m 755 %{buildroot}%{_lispdir}/%{name}
...
# Emacs files
install -p -m 644 lib/jflex-mode.el %{buildroot}%{_lispdir}/%{name}
install -p -m 644 lib/jflex-mode.elc %{buildroot}%{_lispdir}/%{name}
